# Feihong Design System · Scene: Console / Admin
# 飞鸿品牌设计系统 · 场景规范：后台控制台 / 功能页 / 看板 / 仪表盘

> 本规范定义飞鸿品牌设计系统在「后台控制台 / 管理后台 / 功能页 / 看板 / 仪表盘 / 工具页」中的具体应用方式。
> 所有规则建立在 `DNA.md`、`components/14-layouts.md`、`components/` 基础之上，场景专属规则以本文件为准。
> Console场景的核心使命是**可用与高效**——每一个交互都应为"让用户高效完成管理任务"服务。
...
Console场景适用于以下以**管理操作和数据监控为核心目标**的后台界面形态：

| 类型 | 说明 | 典型特征 | 核心任务 |
|------|------|----------|---------|
| **管理后台** | CMS、CRM、数据管理系统 | 表格+表单+侧边栏 | CRUD操作 |
| **看板/仪表盘** | 数据看板、监控面板、运营后台 | 信息密度高，KPI卡片+图表 | 浏览数据+快速操作 |
| **工具页/功能页** | 配置器、数据工具、报表中心 | 单功能聚焦，操作密集 | 完成一次操作 |
| **控制台终端** | 运维控制台、开发者工具后台 | 终端面板+日志+命令交互 | 系统管理与监控 |
...
控制台型页面通常采用**导航区 + 工作区**的经典功能布局，根据页面复杂度有三种典型结构：

```
┌─────────────────────────────────────────────────────────┐
│  结构 A：侧边栏 + 主内容区（最常用，适合多模块后台）         │
├──────────┬──────────────────────────────────────────────┤
│          │  顶部栏（面包屑 + 全局操作 + 用户头像）          │
│          ├──────────────────────────────────────────────┤
│  侧边栏   │                                              │
...
6. **不使用Landing页的section结构**：控制台页不是纵向滚动的营销页，是功能分区的工作界面

---

## 3. 色彩使用指南

Console场景中三色的使用原则与Landing页截然不同——**克制、精准、语义化**。界面的80%以上面积应由中性色承载，品牌色仅承担导航、主操作和状态语义。
...
> **铁律**：酒红在Console场景中是**语义色**而非装饰色，绝不能用于非错误/危险场景的大面积着色。

### 3.4 背景与中性色（Console场景主力色）
...
Console场景的排版以**清晰、高效、易扫读**为核心，字号偏小但层次分明，不使用Landing页的极端字号对比。
...
Console场景使用功能型布局，从layouts中选取以下组合，**不使用L8/L16等艺术化布局**。
...
| **顶部导航+内容** | 单功能工具、轻量工具、配置器 | 顶部固定导航栏 + 下方全宽内容区 |
...
| 布局/做法 | Console场景不推荐原因 |
|-----------|------------------|
| **L8 引文全屏/L16 画册溢出** | 艺术化布局打断工作流，不适合功能界面 |
| **Landing式纵向section** | 控制台页不是营销页，不需要"英雄区→功能区→CTA"结构 |
...
| 组件 | 使用场景 |
|------|----------|
| `sidebar`（侧边栏导航） | 多模块后台必备，含折叠/展开态 |
| `navbar`（顶部栏） | 所有控制台页必备，含面包屑/搜索/用户 |
...
| `btn-cta`（酒红CTA按钮） | 酒红在控制台中仅用于危险/错误，做主按钮会引发焦虑 |
...
控制台/后台/仪表盘是以任务完成和管理操作为核心的功能型界面，用于数据看板、管理后台、运营工具、开发者工具后台、SaaS管理端等用户需要频繁操作和高效完成管理任务的场景。适用于：数据仪表盘、CMS管理后台、运营控制台、项目管理工具、表单密集型应用、内部管理系统。不适用于：营销宣传页（用Landing）、内容阅读（用Tutorial）、作品展示（用Portfolio）。核心原则是"功能优先、效率至上"——品牌色克制使用（80%中性色），信息密度高但不混乱，交互反馈即时明确，长时间使用不疲劳。

## 推荐节奏（Section 序列）

Console场景的"节奏"不是上下滚动的section，而是固定布局框架下的信息层级和模块组织：

### 方案 A：经典侧边栏+仪表盘型（最常用，数据/管理类后台）
...
Console场景装饰极度克制，仅在以下位置使用品牌元素：

- **#06 Terminal Code Block 终端代码块**：开发者工具/日志区域使用墨黑底+JetBrains Mono+终端绿#4ADE80+酒红错误文字，这是Console场景最有辨识度的元素
- **#14 Section Marker 金色选中条**：侧边栏当前选中项左侧3px金色指示条（`width:3px; background:var(--gold)`），是Console场景最核心的品牌签名
...
Console场景色彩策略核心：80%面积为中性色（暖米白/白/浅暖灰），品牌色仅用于交互和状态标记，功能界面不搞"视觉冲击"。
...
- 数据下降趋势箭头/数字
- 危险操作二次确认的确认按钮
- **酒红在控制台中=警告/危险/错误，绝不用于正常主按钮（会引发焦虑）**
...
**终端绿 #4ADE80（特殊色）**：仅用于代码/终端区域的命令提示符和成功输出，不属于品牌三色但为Console场景专用。

## 场景禁忌（Don'ts）

1. **酒红作为主按钮颜色**：控制台中酒红=危险/删除，主按钮用皇家蓝，否则用户会对正常操作产生焦虑
...
*Scene: Console v1.0 · Feihong Design System*
